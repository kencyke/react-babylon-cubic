...
export class BoundingBoxGizmo2D extends Gizmo {
    ...
    private _surface = Surface.ZX // TODO: change properly
    ...
    constructor(
        ...
        for (let i = 0; i < 2; i++) {
            for (let j = 0; j < 2; j++) {
                ...
                // Dragging logic
                let dragAxes: Array<Vector3>
                if (surface === Surface.XY) {
                    dragAxes = [
                        new Vector3(i === 0 ? 1 : -1, 0, 0),
                        new Vector3(0, j === 0 ? 1 : -1, 0)
                    ]
                }
                else if (surface === Surface.YZ) {
                    dragAxes = [
                        new Vector3(0, i === 0 ? 1 : -1, 0),
                        new Vector3(0, 0, j === 0 ? 1 : -1)
                    ]
                }
                else { // ZX
                    dragAxes = [
                        new Vector3(0, 0, i === 0 ? 1 : -1),
                        new Vector3(j === 0 ? 1 : -1, 0, 0)
                    ]
                }
                dragAxes.forEach(dragAxis => {
                    ...
                    _dragBehavior.onDragObservable.add(event => {
                        ...
                        if (this.attachedMesh) {
                            const sign = dragAxis.x + dragAxis.y + dragAxis.z // 1 or -1
                            ...
                            if (this.uniformScaling) {
                                ...
                                if (sign < 0) {
                                    ...
                                    tmpVector.copyFrom(dragAxis).scaleInPlace(-1)
                                }
                            }
                            ...
                            if (snapped) {
                                ...
                                this.onScaleBoxDragEndObservable.notifyObservers(tmpSnapEvent);
                            }
                        }
                        this._updateDummy()
                    })
        ...
        for (let i = 0; i < 2; i++) {
            for (let j = 0; j < 2; j++) {
                ...
                if (scaleBoxes[index]) {
                    ...
                    else if (this._surface === Surface.YZ) {
                        scaleBoxes[index].position.set(
                            0,
                            this._boundingDimensions.y * i,
                            this._boundingDimensions.z * j
                        )
                    }
                    else { // ZX
                        scaleBoxes[index].position.set(
