...
export class BoundingBoxGizmoXY extends Gizmo {
  ...
  constructor(
    ...
    gizmoLayer: UtilityLayerRenderer = UtilityLayerRenderer.DefaultKeepDepthUtilityLayer
  ) {
    super(gizmoLayer)
    ...
    for (let i = 0; i < 12; i++) {
      const sphere = SphereBuilder.CreateSphere("", { diameter: 1 }, gizmoLayer.utilityLayerScene)
      //sphere.rotationQuaternion = new Quaternion()
      sphere.material = this.coloredMaterial

      // Drag behavior
      const xDragAxis: Vector3 = new Vector3(1, 0, 0)
      const yDragAxis: Vector3 = new Vector3(0, -1, 0)
      const xyDragAxes = new Array<Vector3>(yDragAxis)

      xyDragAxes.forEach(dragAxis => {
        ...
        let tmpSnapEvent = { snapDistance: 0 };
        _dragBehavior.onDragObservable.add(event => {
          if (this.attachedMesh) {
            ...
            if (this.snapDistance == 0) {
              tmpVector.scaleToRef(dragStrength, tmpVector); // XXX
            } else {
          ...
          //   if (
          //     originalParent &&
          //     (originalParent as Mesh).scaling &&
          //     (originalParent as Mesh).scaling.isNonUniformWithinEpsilon(0.001)
          //   ) {
          ...
          //   let projectDist =
          //     Vector3.Dot(dragAxis, event.delta) < 0 ? Math.abs(event.delta.length()) : -Math.abs(event.delta.length())
    ...
    for (let i = 0; i < 2; i++) {
      for (let j = 0; j < 2; j++) {
        for (let k = 0; k < 2; k++) {
          const box = BoxBuilder.CreateBox("", { size: 1 }, gizmoLayer.utilityLayerScene)
          box.material = this.coloredMaterial

          // Dragging logic
          const dragAxis = new Vector3(i === 0 ? -1 : 1, j === 0 ? -1 : 1, k === 0 ? -1 : 1)
          const _dragBehavior = new PointerDragBehavior({ dragAxis: dragAxis })
          _dragBehavior.moveAttached = false
          ...
          _dragBehavior.onDragObservable.add(event => {
            ...
            if (this.attachedMesh) {
              ...
              const relativeDragDistance =
                (event.dragDistance / this._boundingDimensions.length()) * this._anchorMesh.scaling.length()
    ...
    this._pointerObserver = gizmoLayer.utilityLayerScene.onPointerObservable.add(pointerInfo => {
      if (!pointerIds[(pointerInfo.event as PointerEvent).pointerId]) {
        this._rotateSpheresParent
          .getChildMeshes()
          .concat(this._scaleBoxesParent.getChildMeshes())
          .forEach(mesh => {
            if (pointerInfo.pickInfo && pointerInfo.pickInfo.pickedMesh === mesh) {
              pointerIds[(pointerInfo.event as PointerEvent).pointerId] = mesh
        ...
        if (
          ...
          pointerInfo.pickInfo.pickedMesh !== pointerIds[(pointerInfo.event as PointerEvent).pointerId]
        ) {
          pointerIds[(pointerInfo.event as PointerEvent).pointerId].material = this.coloredMaterial
          delete pointerIds[(pointerInfo.event as PointerEvent).pointerId]
        }
      }
    })
  ...
  public updateBoundingBox() {
    if (this.attachedMesh) {
      ...
      let originalSkeletonOverrideMesh = null
  ...
  public static MakeNotPickableAndWrapInBoundingBox(mesh: Mesh): Mesh {
    ...
    // Update bounding dimensions/positions
    const box = BoxBuilder.CreateBox("box", { size: 1 }, mesh.getScene())
